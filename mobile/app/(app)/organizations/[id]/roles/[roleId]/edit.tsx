...
import { PermissionCheckbox, PermissionState } from '../../../../../../components/specialized';
import { RESERVED_ROLES, type DefaultRoleName } from '../../../../../../src/lib/permissions';
...
export default function EditRoleScreen() {
  ...
  const loadRoleData = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/organizations/${orgId}/roles/${roleId}`, {
        method: 'GET',
        credentials: 'include',
      });
  ...
  const renderPermissionChanges = () => {
    ...
    return (
      <View style={styles.changesContainer}>
        <Badge variant={currentCount > initialCount ? 'default' : 'destructive'}>
          {initialCount} → {currentCount} permissions
        </Badge>
        <Badge style={[styles.unsavedBadge, { backgroundColor: colors.warning + '20', borderColor: colors.warning }]}>
          Unsaved changes
        </Badge>
      </View>
    );
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        <Text style={[styles.title, { color: colors.foreground }]}>Edit Role</Text>
        <View style={styles.roleNameRow}>
          <Text style={[styles.roleName, { color: colors.primary }]}>{roleData.role}</Text>
          {reserved && (
            <Badge style={styles.reservedBadge}>System Role</Badge>
          )}
        </View>
        {reserved && (
          <Text style={[styles.reservedNote, { color: colors.mutedForeground }]}>
            System roles have predefined permissions. Changes will apply to all members with this role.
          </Text>
        )}
      </View>
      ...
      <ScrollView style={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Card padding="lg" style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Role Information</Text>
          <View style={styles.infoRow}>
            <Text style={[styles.infoLabel, { color: colors.mutedForeground }]}>Role Name:</Text>
            <Text style={[styles.infoValue, { color: colors.foreground }]}>{roleData.role}</Text>
          </View>
          <View style={styles.infoRow}>
