...
import { useInvalidateOrgQueries } from "@/hooks/use-organization-queries";

type AllActions = "create" | "read" | "update" | "delete" | "cancel";

function isValidActionForResource(
  resource: Resource,
  action: string,
): action is AllActions {
...
export function RoleForm({
  ...
  const togglePermission = (resource: Resource, action: string) => {
    if (!isValidActionForResource(resource, action)) return;

    setFormData((prev) => ({
      ...prev,
      permissions: toggleActionInPermissions(
        prev.permissions,
        resource,
        action,
      ),
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.name.trim()) {
      ...
      RESERVED_ROLES.includes(
        formData.name.toLowerCase() as (typeof RESERVED_ROLES)[number],
      )
    ) {
    ...
    try {
      if (isEditing && existingRole) {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_AUTH_URL}/api/organizations/${organizationId}/roles/${existingRole.id}`,
          {
            method: "PUT",
            credentials: "include",
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant={getPermissionCount() > 0 ? "default" : "outline"}
                >
                  {getPermissionCount()} permission
                  {getPermissionCount() !== 1 ? "s" : ""} selected
                </Badge>
              </div>
            </CardHeader>
            <CardContent>
              ...
              <Accordion type="multiple" defaultValue={[]}>
                {(Object.keys(RESOURCES) as Resource[]).map((resource) => {
                  ...
                  const isPartiallyChecked =
                    isResourcePartiallyChecked(resource);
                    ...
                    <AccordionItem key={resource} value={resource}>
                      <AccordionTrigger className="hover:no-underline">
                        <div className="flex items-center gap-3">
                          ...
                          />
                          <Icon
                            className={`h-5 w-5 ${RESOURCE_COLORS[resource]}`}
                          />
                          <span className="capitalize font-medium">
                            {resource}
                          </span>
                          <Badge variant="outline" className="ml-2">
                            {getResourcePermissionCount(resource)}/
                            {actions.length}
                          </Badge>
                        </div>
                      </AccordionTrigger>
                      <AccordionContent>
                        <div className="ml-8 space-y-3 pt-2">
                          {actions.map((action) => {
                            const isChecked =
                              isValidActionForResource(resource, action) &&
                              hasAction(formData.permissions, resource, action);
                            return (
                              <div
                                key={action}
          ...
          {!isEditing && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Start with a pre-configured role
                </CardDescription>
              </CardHeader>
              ...
              <div className="space-y-3">
                {(Object.keys(RESOURCES) as Resource[]).map((resource) => {
                  ...
                  return (
                    ...
                    >
                      <Icon
                        className={`h-4 w-4 ${RESOURCE_COLORS[resource]}`}
                      />
