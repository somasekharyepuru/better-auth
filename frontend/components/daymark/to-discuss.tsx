...
export function ToDiscuss({
  ...
  const handleAdd = async () => {
    ...
    setIsLoading(true);
    try {
      const newItem = await discussionItemsApi.create(
        date,
        newContent.trim(),
        lifeAreaId,
      );
    ...
    <div className="card-premium">
      ...
      <div className="space-y-3">
        {localItems.map((item, index) => (
          ...
          >
            <div className="group flex items-start gap-4 p-4 rounded-2xl hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-all">
              {/* Number badge */}
