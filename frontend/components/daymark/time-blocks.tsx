...
import {
  ...
  DEFAULT_TIME_BLOCK_TYPES,
  getColorClasses,
} from "@/lib/time-block-types-context";
...
export function TimeBlocks({
  ...
  onUpdate,
  defaultDuration = 60,
  defaultType = "Deep Work",
  lifeAreaId,
  readOnly = false,
}: TimeBlocksProps) {
  const typesContext = useTimeBlockTypesOptional();
  const [localBlocks, setLocalBlocks] = useState<TimeBlock[]>(blocks);
  const [isAdding, setIsAdding] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);

  // Get available types from context or use defaults
  // Use defaults if context is not available OR if context has no active types (loading/error state)
  const availableTypes =
    typesContext?.activeTypes && typesContext.activeTypes.length > 0
      ? typesContext.activeTypes.map((t) => t.name)
      : DEFAULT_TIME_BLOCK_TYPES;

  // Get color for a type
  const getTypeColorClass = (typeName: string) => {
    if (typesContext) {
      return typesContext.getTypeColor(typeName);
    }
    return FALLBACK_TYPE_COLORS[typeName] || FALLBACK_TYPE_COLORS.Admin;
  };
  ...
  const handleAdd = async () => {
    ...
    try {
      const startTime = new Date(
        `${date}T${formData.startTime}:00`,
      ).toISOString();
      const endTime = new Date(`${date}T${formData.endTime}:00`).toISOString();

      const newBlock = await timeBlocksApi.create(
        date,
        {
          ...
          {sortedBlocks.length === 0 ? (
            ...
            sortedBlocks.map((block) => (
              ...
              >
                ...
                {/* Type badge */}
                <span
                  className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${getTypeColorClass(block.type)}`}
                >
                  {block.type}
                </span>

                {/* Title */}
                <span className="flex-1 text-gray-900 dark:text-gray-100 truncate min-w-0">
                  {block.title}
                </span>
              </div>
            ))
  ...
  return (
    <div className="card-premium">
      ...
      <div
        className={`space-y-2 max-h-[400px] pr-1 ${isAdding || editingId ? "overflow-visible" : "overflow-y-auto"}`}
      >
        {sortedBlocks.map((block) =>
          editingId === block.id ? (
            <TimeBlockForm
