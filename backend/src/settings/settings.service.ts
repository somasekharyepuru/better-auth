...
export class SettingsService {
  ...
  async updateSettings(
    ...
    if (data.maxDiscussionItems !== undefined) {
      data.maxDiscussionItems = Math.max(
        0,
        Math.min(5, data.maxDiscussionItems),
      );
    }
    if (data.defaultTimeBlockDuration !== undefined) {
      data.defaultTimeBlockDuration = Math.max(
        15,
        Math.min(240, data.defaultTimeBlockDuration),
      );
    }
    // Validate pomodoro durations
    if (data.pomodoroFocusDuration !== undefined) {
      data.pomodoroFocusDuration = Math.max(
        1,
        Math.min(120, data.pomodoroFocusDuration),
      );
    }
    if (data.pomodoroShortBreak !== undefined) {
      data.pomodoroShortBreak = Math.max(
        1,
        Math.min(30, data.pomodoroShortBreak),
      );
    }
    if (data.pomodoroLongBreak !== undefined) {
      data.pomodoroLongBreak = Math.max(
        1,
        Math.min(60, data.pomodoroLongBreak),
      );
    }
  ...
  private formatSettings(settings: any): UserSettingsResponse {
    return {
      ...
      focusBlocksCalendar: settings.focusBlocksCalendar,
      // Life Areas settings
