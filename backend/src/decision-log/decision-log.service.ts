...
export interface DecisionWithRelations {
  ...
  focusSessions?: {
    focusSession: { id: string; startedAt: Date; completed: boolean };
  }[];
}
...
export class DecisionLogService {
    ...
    const data = await this.prisma.decisionEntry.findMany({
      ...
      include: {
        ...
        focusSessions: {
          include: {
            focusSession: {
              select: { id: true, startedAt: true, completed: true },
            },
          },
        },
      },
  ...
  async createDecision(
    ...
    return this.getDecision(
      decision.id,
      userId,
    ) as Promise<DecisionWithRelations>;
  }
