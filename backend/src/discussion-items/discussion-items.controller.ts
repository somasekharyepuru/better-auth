...
export class DiscussionItemsController {
  constructor(
    private readonly discussionItemsService: DiscussionItemsService,
  ) {}
  ...
  async createItem(
    ...
    return this.discussionItemsService.createItem(
      userId,
      date,
      body.content,
      body.lifeAreaId,
  ...
  async moveItem(
    ...
    const userId = this.getUserIdFromRequest(req);
    return this.discussionItemsService.moveToLifeArea(
      id,
      userId,
      body.targetLifeAreaId,
      body.date,
    );
  }
}
