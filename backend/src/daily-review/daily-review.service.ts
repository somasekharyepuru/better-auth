...
export class DailyReviewService {
   ...
   */
  async carryForwardPriorities(
    userId: string,
    fromDateStr: string,
    toDateStr: string,
    lifeAreaId?: string,
  ) {
    const fromDay = await this.daysService.getOrCreateDay(
      userId,
      fromDateStr,
      lifeAreaId || null,
    );
    const toDay = await this.daysService.getOrCreateDay(
      userId,
      toDateStr,
      lifeAreaId || null,
    );
    ...
    const highestOrder =
      existingPriorities.length > 0 ? existingPriorities[0].order : 0;
