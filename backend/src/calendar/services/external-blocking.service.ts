    ...
    for (const source of sources) {
      try {
        ...
        const eventInput: CreateEventInput = {
          title: `[Focus] ${title}`,
          description:
            "Focus time blocked by Daymark - please do not schedule during this time",
          startTime,
          endTime,
          isAllDay: false,
        };
        ...
        this.logger.log(
          `Created blocking event on ${source.name} for time block ${timeBlockId}`,
        );
      } catch (error) {
        this.logger.error(
          `Failed to create blocking event on ${source.name}: ${error}`,
        );
  ...
  async removeBlockingEvents(timeBlockId: string): Promise<void> {
    ...
    for (const mapping of blockingMappings) {
      try {
        ...
        this.logger.log(
          `Removed blocking event from ${source.name} for time block ${timeBlockId}`,
        );
      } catch (error) {
  ...
  async updateBlockingEvents(
    ...
    for (const mapping of blockingMappings) {
      try {
        ...
        this.logger.log(
          `Updated blocking event on ${source.name} for time block ${timeBlockId}`,
        );
      } catch (error) {
