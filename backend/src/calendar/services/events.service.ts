...
export class CalendarEventsService {
  ...
  async createEvent(
    ...
    try {
      ...
      this.logger.log(
        `Created event ${createdEvent.id} in ${source.connection.provider}`,
      );
    } catch (error) {
    ...
    if (mapping && mapping.externalEventId && mapping.calendarSource) {
      ...
      if (["BIDIRECTIONAL", "WRITE_ONLY"].includes(source.syncDirection)) {
        try {
          ...
          this.logger.log(
            `Updated event ${mapping.externalEventId} in ${source.connection.provider}`,
          );
        } catch (error) {
    ...
    if (mapping && mapping.externalEventId && mapping.calendarSource) {
      ...
      if (["BIDIRECTIONAL", "WRITE_ONLY"].includes(source.syncDirection)) {
        try {
          ...
          this.logger.log(
            `Deleted event ${mapping.externalEventId} from ${source.connection.provider}`,
          );
        } catch (error) {
          this.logger.error(
            `Failed to delete event from external calendar: ${error}`,
          );
