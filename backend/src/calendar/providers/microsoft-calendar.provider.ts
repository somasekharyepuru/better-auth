...
export class MicrosoftCalendarProvider implements ICalendarProvider {
  ...
  async refreshAccessToken(refreshToken: string): Promise<OAuthTokens> {
    ...
    if (data.error) {
      throw new Error(
        `Microsoft token refresh error: ${data.error_description}`,
      );
    }
  ...
  async getEvents(
    ...
    try {
      ...
      return {
        events: response.value.map((e: Record<string, unknown>) =>
          this.mapEvent(e),
        ),
      ...
      if (err.statusCode === 410) {
        this.logger.warn(
          `Delta token expired for calendar ${calendarId}, performing full sync`,
        );
  ...
  private mapEvent(event: Record<string, unknown>): ExternalEvent {
    ...
    return {
      ...
      visibility:
        (event.sensitivity as string) === "private" ? "private" : "public",
