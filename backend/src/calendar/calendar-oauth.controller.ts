...
export class CalendarOAuthController {
  ...
  async googleCallback(
    ...
    if (error) {
      ...
      return res.redirect(
        `${frontendUrl}/settings/calendars?error=${encodeURIComponent(error)}`,
      );
    }

    if (!code || !state) {
      return res.redirect(
        `${frontendUrl}/settings/calendars?error=missing_params`,
      );
    }

    try {
      const redirectUri = `${process.env.BETTER_AUTH_URL || "http://localhost:3002"}/api/calendar/oauth/google/callback`;
      const { connectionId } = await this.connectionService.completeConnection(
        state,
        code,
        redirectUri,
      );
      ...
      return res.redirect(
        `${frontendUrl}/settings/calendars?success=true&provider=google`,
      );
    } catch (err) {
      this.logger.error("Failed to complete Google connection:", err);
      return res.redirect(
        `${frontendUrl}/settings/calendars?error=connection_failed`,
      );
    }
  }
  ...
  async microsoftCallback(
    ...
    try {
      ...
      this.logger.log(
        `Microsoft calendar connection completed: ${connectionId}`,
      );
      return res.redirect(
        `${frontendUrl}/settings/calendars?success=true&provider=microsoft`,
      );
    } catch (err) {
