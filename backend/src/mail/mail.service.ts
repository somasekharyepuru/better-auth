...
export class MailService {
  ...
  async sendEmail(email: string, otp: string, type: string): Promise<boolean> {
    const maskedOtp =
      typeof otp === "string" && otp.length > 2
        ? `${otp.slice(0, 2)}****`
        : "******";
    let attempt = 0;
    ...
    while (attempt < this.maxRetries) {
      ...
      try {
        ...
        this.logger.log(`[MailService] Sent email (OTP: ${maskedOtp})`, {
          type,
          email,
          attempt,
        });
        ...
        this.logger.warn(`[MailService] Attempt failed, retrying`, {
          type,
          email,
          attempt,
          backoff,
  ...
  async healthCheck({
    ...
    try {
      const controller = new AbortController();
      const timeout = setTimeout(
        () => controller.abort(new Error("Mail webhook health timeout")),
        timeoutMs,
      );
