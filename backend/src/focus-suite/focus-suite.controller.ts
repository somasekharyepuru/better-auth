...
import {
  FocusSuiteMatrixService,
  CreateFocusBlockFromMatrixInput,
} from "./focus-suite-matrix.service";
...
export class FocusSuiteController {
  ...
  async getWeeklyDecisions(
    ...
    const userId = this.getUserIdFromRequest(req);
    return this.analyticsService.getWeeklyDecisionSummary(
      userId,
      new Date(weekStart),
    );
  }
  ...
  async getSessionStats(
    ...
    const startDate = new Date(start);
    const endDate = new Date(end);
    return this.analyticsService.getFocusSessionStats(
      userId,
      startDate,
      endDate,
    );
