...
export class TimeBlocksService {
  ...
  async createTimeBlock(
    ...
    if (!TIME_BLOCK_CATEGORIES.includes(category as TimeBlockCategory)) {
      throw new BadRequestException(
        `Invalid category. Must be one of: ${TIME_BLOCK_CATEGORIES.join(", ")}`,
      );
    }
    ...
    const shouldBlock =
      (category === "focus" || category === "deep-work") &&
      ...
      this.externalBlockingService
        ...
        .then((result) => {
          if (result.blockedCalendars.length > 0) {
            this.logger.log(
              `Created blocking events on: ${result.blockedCalendars.join(", ")}`,
            );
          }
          if (result.errors.length > 0) {
            this.logger.warn(
              `Some blocking events failed: ${result.errors.join("; ")}`,
            );
          }
        })
        .catch((error) => {
  ...
  async deleteTimeBlock(blockId: string, userId: string) {
    ...
    if (
      block.blockExternalCalendars &&
      (block.category === "focus" || block.category === "deep-work")
    ) {
      // Fire and forget - don't block the response
      this.externalBlockingService
        .removeBlockingEvents(blockId)
        .catch((error) => {
