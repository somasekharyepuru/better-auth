...
export class TimeBlocksController {
  ...
  async getTimeBlocksForDate(
    ...
    return this.timeBlocksService.getTimeBlocksForDate(
      userId,
      date,
      lifeAreaId,
    );
  ...
  async getTimeBlocksForRange(
    ...
    return this.timeBlocksService.getTimeBlocksForRange(
      userId,
      startDate,
      endDate,
      lifeAreaId,
  ...
  async getTimeBlocksByCategory(
    ...
    const startDate = new Date(start);
    const endDate = new Date(end);
    return this.timeBlocksService.getTimeBlocksByCategory(
      userId,
      category,
      startDate,
      endDate,
  ...
  async checkConflicts(
    @Body()
    body: { startTime: string; endTime: string; excludeBlockId?: string },
    @Req() req: Request,
  ) {
    const userId = this.getUserIdFromRequest(req);
    const startTime = new Date(body.startTime);
    const endTime = new Date(body.endTime);
    return this.timeBlocksService.checkConflicts(
      userId,
      startTime,
      endTime,
      body.excludeBlockId,
  ...
  async createTimeBlock(
    ...
    return this.timeBlocksService.createTimeBlock(
      userId,
      date,
      data,
      lifeAreaId,
  ...
  async unlinkFromPriority(@Param("id") id: string, @Req() req: Request) {
    const userId = this.getUserIdFromRequest(req);
