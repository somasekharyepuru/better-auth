...
if [ $tries -eq 10 ]; then
  echo "WARNING: Could not verify database connection, but proceeding..."
...
if [ "$has_migrations" = true ]; then
  ...
  if [ $tries -eq $max_tries ]; then
    ...
    echo "WARNING: Database may be in inconsistent state"
    exit 1
  fi
else
  echo "WARNING: No migrations found. Using db push (development only)."
  echo "For production, create migrations first: npm run db:migrate"
  ...
  if [ $tries -eq $max_tries ]; then
    ...
    echo "This may indicate schema conflicts. Create migrations instead."
    exit 1
  fi
fi
...
echo "Checking for admin user seed..."
if [ -n "$ADMIN_EMAIL" ] && [ -n "$ADMIN_PASSWORD" ]; then
  echo "Admin credentials found, running seed..."
  npx ts-node --transpile-only src/seed-admin.ts || echo "Warning: Admin seed failed, continuing..."
else
  echo "No admin credentials configured, skipping seed."
fi
